...
public class TalkRepositoryImpl implements TalkRepository {
    ...
    public List<Talk> findAll() {
        ...
        try (Connection connection = DATA_SOURCE.getConnection()) {
            try (PreparedStatement statement = connection.prepareStatement(
                    "SELECT * FROM Talk ORDER BY creationTime DESC, id")) {
    ...
    public List<Talk> findAllIncoming(long userId) {
        ...
        try (Connection connection = DATA_SOURCE.getConnection()) {
            try (PreparedStatement statement = connection.prepareStatement(
                    "SELECT * FROM Talk WHERE targetUserId=? ORDER BY creationTime DESC, id")) {
            ...
            throw new RepositoryException("Can't find all incoming talks.", e);
        }
        return talks;
    }
    ...
    public List<Talk> findAllOutcoming(long userId) {
        ...
        try (Connection connection = DATA_SOURCE.getConnection()) {
            try (PreparedStatement statement = connection.prepareStatement(
                    "SELECT * FROM Talk WHERE sourceUserId=? ORDER BY creationTime DESC, id")) {
                statement.setLong(1, userId);
                try (ResultSet resultSet = statement.executeQuery()) {
                    while (resultSet.next()) {
                        talks.add(toTalk(statement.getMetaData(), resultSet));
            ...
            throw new RepositoryException("Can't find all outcoming talks.", e);
        }
        return talks;
    }
    ...
    private Talk toTalk(ResultSetMetaData metaData, ResultSet resultSet) throws SQLException {
        ...
        for (int i = 1; i <= metaData.getColumnCount(); i++) {
            ...
            if ("id".equalsIgnoreCase(columnName)) {
                ...
                talk.setText(resultSet.getString(columnName));
